...
Namespace CustomDataSourceWizard
	Public Class WizardDataModel
		...
		Private Function OpenDataSourceEditor(ByVal dsw As DataSourceWrapper) As Boolean
			...
			Dim okCommand As UICommand = New UICommand With {.Caption = "OK", .IsCancel = False, .IsDefault = True, .Command = New DelegateCommand(Of CancelEventArgs)(Function(x As CancelEventArgs)
				
			End Function, Function(x As CancelEventArgs)
				Return String.IsNullOrEmpty(dataErrorInfo.Error)
			End Function)}
			Dim cancelCommand As UICommand = New UICommand With {
		...
		Private Function OpenQueryEditor(ByVal qw As QueryWrapper) As Boolean
			Dim okCommand = New UICommand With {.Caption = "OK", .IsCancel = False, .IsDefault = True, .Command = New DelegateCommand(Of CancelEventArgs)(Function(x As CancelEventArgs)
				
			End Function, Function(x As CancelEventArgs)
				Return String.IsNullOrEmpty(DirectCast(qw, IDataErrorInfo).Error)
			End Function)}
